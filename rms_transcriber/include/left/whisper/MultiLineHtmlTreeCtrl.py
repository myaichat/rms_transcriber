...
class MultiLineHtmlTreeCtrl(CT.CustomTreeCtrl):
    ...
    def on_stream_recognized(self, data):
        ...
        if transcript.strip():
            item_id=f'{tid}:{rid}'
            wx.CallAfter(self.on_append_recognized_stream, item_id, transcript)
    ...
    def on_append_recognized_stream(self, item_id, transcript):
        ...
        if transcript.strip():
            ...
            recog_item.SetBackgroundColour(wx.Colour(240, 240, 240))
    ...
    def on_partial_stream(self, data):  
        ...
        if transcript.strip():
            ...
            if item_id in self.html_items:
                ...
                self.append_tree_with_transcript(item_id, f'{item_id}, {transcript}')       
    ...
    def close_stream(self,item_id, transcript):
        print('close_stream', item_id, transcript)
        if item_id in self.html_items:
    ...
    def recreate_html_item(self, item_id, transcript):
        ...
        if item_id in self.html_items:
            ...
            print('recreate_html_item: Item ID not found in html_items', item_id, transcript)
    ...
    def AppendMultilineItem(self, item_id, parent, text_item, data=None, pad_item=True):
        ...
        if  item_id not in self.html_items:
            ...
            print('HTMLTREE: AppendMultilineItem: Item ID already exists:', item_id, text_item)
